...
void enterI2cCriticalError() {
  ...
  // Без yield/delay WDT срабатывает через ~8 сек.
...
// При сбое PCF8574 выполняется 2 попытки recoverI2C() с паузой 500 мс,
// после чего выполняется перезагрузка.

// Делает до maxRetries попыток recoverI2C() с паузой pauseMs между ними.
// Если устройство не отвечает — вызывает enterI2cCriticalError() -> HW WDT.
static void waitForPCF8574OrReboot(uint8_t maxRetries = 2,
  ...
  // Все попытки провалились — перезагружаемся
  logEvent("PCF8574 не відновлено. Перезапуск!");
