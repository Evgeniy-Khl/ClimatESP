...
void prepareTable(uint8_t prg, uint8_t day, uint8_t amountday, int16_t t0, int16_t t1, 
  int16_t rh, int16_t timer, int16_t aer0, int16_t aer1, int16_t fl){
    ...
    for (size_t i = 0; i < amountday; i++){
        day += i;
        uint16_t memoryAddress = eepromMemoryAddressForDay(prg, day);
        // byte res = eepromWrBuff(memoryAddress, unBuf.buffer, sizeof(unBuf));
        DEBUG_PRINT("DAY:"); DEBUG_PRINT(day); DEBUG_PRINT("; ADD:"); DEBUG_PRINTLN(memoryAddress);
    }
...
void prepareProg2(){
    prepareTable(1, 1, 8,380,330,700,60, 0, 0, 0);// 1-8	38,0 оС	33,0 оС(70,0%)	закрита	увімкнено	вимкнуто
    prepareTable(1, 9, 5,375,310,600,60,10,10,15);// 9-13	37,5 оС	31,0 оС(60,0%)	15%	увімкнено	1 раз 5 хв.
    prepareTable(1,14,11,372,300,560,60,10,10,25);// 14-24	37,2 оС	30,0 оС(56,0%)	25%	увімкнено	2 рази 20 хв.
    prepareTable(1,25, 4,370,320,700, 0,10,10,40);// 25-28	37,0 оС	32,0 оС(70,0%)	40%	вимкнений	1 раз 10 хв.
    prepareTable(1,29, 2,280,200,450, 0, 0, 0,90);// 29-31	28,0 оС	20,0 оС(45,0%)	90%	вимкнений	вимкнуто
}

// Інкубація перепелиних яєць.
void prepareProg3(){
    prepareTable(1, 1,12,376,310,580,60, 0, 0, 0);// 1-12	37,6 оС	31,0 оС(58,0%)	закрита	увімкнено	вимкнуто
    prepareTable(1,13, 3,373,290,530,60,10,10, 5);// 13-15	37,3 оС	29,0 оС(53,0%)	5 %	увімкнено	вимкнуто
    prepareTable(1,16, 2,372,280,470, 0,10,10,15);// 16-17	37,2 оС	28,0 оС(47,0%)	15%	вимкнений	вимкнуто
    prepareTable(1,18, 2,370,340,800, 0,10,10,20);// 18-19	37,0 оС	34,0 оС(80,0%)	20%	вимкнений	вимкнуто
    prepareTable(1,20,11,280,200,450, 0, 0, 0,90);// 20-31	28,0 оС	20,0 оС(45,0%)	90%	вимкнений	вимкнуто
}

// Інкубація індикових яєць.
void prepareProg4(){
    prepareTable(1, 1, 6,378,300,560,60, 0, 0, 0);// 1-6	37,8 оС	30,0 оС(56,0%)	закрита	увімкнено	вимкнуто
    prepareTable(1, 7, 6,375,290,520,60,10,10,15);// 7-12	37,5 оС	29,0 оС(52,0%)	15 %	увімкнено	вимкнуто
    prepareTable(1,13,14,372,288,520,60,10,10,25);// 13-26	37,2 оС	28,8 оС(52,0%)	25%	увімкнено	вимкнуто
    prepareTable(1,27, 2,370,320,700, 0,10,10,35);// 27-28	37,0 оС	32,0 оС(70,0%)	35%	вимкнений	вимкнуто
    prepareTable(1,29, 2,280,200,450, 0, 0, 0,90);// 29-31	28,0 оС	20,0 оС(45,0%)	90%	вимкнений	вимкнуто
}